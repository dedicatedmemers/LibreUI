...
public final class BookOpenPluginMessageHandler {
     ...
     */
    private static final byte[] MAIN_HAND_ENUM_CONSTANT_BYTE_ARRAY
            = new byte[] {MAIN_HAND_ENUM_CONSTANT_INDEX};
    ...
    private static final PlayerChannelRegistrationInsurer
            CHANNEL_REGISTRATION_INSURER =
            new PlayerChannelRegistrationInsurer
                    (new FieldCachingChannelsRetrieveStrategy());
    ...
    public void registerBookOpenMessagingChannel() {
        this.plugin.getServer().getMessenger()
                .registerOutgoingPluginChannel(this.plugin, OPEN_BOOK_MESSAGE_CHANNEL);
    }
    ...
    public void sendOpenBookMessageToPlayer(Player player, Version version) {
        ...
        CHANNEL_REGISTRATION_INSURER
                .insureChannelRegistration(player, OPEN_BOOK_MESSAGE_CHANNEL);
    ...
    private static byte[] getAppropriateMessageFromVersion(Version version) {
        return version.compareTo(DUAL_WIELDING_VERSION_DIVIDER) >= 0
                ? MAIN_HAND_ENUM_CONSTANT_BYTE_ARRAY
                : EMPTY_BYTE_ARRAY;
    }
}
